...
| 활동 보너스 | 주간 검증 사용량 `5,000,000`토큰에서 최대치 도달 |
| ORIGIN 천장 | 300회 연속 미등장 시 다음 301번째 가챠에서 확정 |
| 유니크 컬러 | 등급과 별도로 매 가챠 `0.1%`, 능력 차이와 천장 없음 |
| 중복 크리처 | 자동 분해 없이 별도 개체로 보유하고 서로 다르게 육성 가능 |
| 진화 | 먹이·훈련·친밀도·아이템·시간이 후보 확률에 영향을 주며 완료 후 되돌릴 수 없음 |

### 등급 확률

| 등급 | 기본 확률 | 최대 활동 보너스 적용 |
| --- | ---: | ---: |
| PROCESS | 55% | 40% |
| AGENT | 25% | 26% |
| DAEMON | 12% | 16% |
...
활동 보너스는 주간 검증 사용량에 따라 기본 확률과 최대 확률 사이를 선형으로 보간합니다. 위 수치는 알파 운영값이며 플레이 테스트에 따라 조정될 수 있습니다.
...
macOS 팝업에서는 일반·대형 먹이의 구매와 급여 버튼을 길게 눌러 가속 연속 실행할 수 있습니다. `등급 · 확률`에서 현재 활동 보너스를 반영한 등급별 확률표를 확인할 수 있으며, 높은 등급일수록 배지·테두리·오라 효과가 강화됩니다.

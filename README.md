...
<table>
  <tr>
    <td width="33%" valign="top">
      ...
      앱을 설치하고 첫 크리처를 깨웁니다.<br />
      <a href="#quick-start">빠른 시작 →</a>
    </td>
    <td width="34%" valign="top">
      <strong>SCOUT // EXPLORE</strong><br />
      실제 화면, 게임 규칙과 개인정보 경계를 확인합니다.<br />
      <a href="#live-systems">제품 살펴보기 →</a>
    </td>
    <td width="33%" valign="top">
      <strong>BUILDER // CONTRIBUTE</strong><br />
      구조와 검증 절차를 읽고 함께 만듭니다.<br />
      <a href="#contributing">기여 시작하기 →</a>
    </td>
  </tr>
</table>
...
각 진화 갈림길(Lv.15 또는 Lv.25)에서는 2지선다로 진화 방향을 직접 고릅니다. 갈림길에 도달하면 진화가 멈추고 배지로 선택을 요청하며, 고른 방향은 그 진화에만 적용되어 다음 갈림길에서 다시 묻습니다.

서로 관련 없는 크리처 계열을 합친 `mixed` 10종은 동일 개체의 성장으로 볼 수 없어 일반 진화와 갈림길, 성장 잠재력 계산, 진화도감 계보에서 제외한 독립 합성 수집품입니다. 기존 저장에서 이미 보유한 합성종은 안전하게 유지되며, 진화도감에는 부모 계보를 새로 만들지 않고 실제 현재 모습만 표시합니다.

변이 후보가 있는 계보는 Lv.15 진화 순간마다 10% 확률로 변이가 발동해 수락/거절을 묻습니다. 수락하면 그 자리에서 변이체로 종착하고, 거절하면 발동 전에 사용자가 선택했거나 시스템이 자동으로 정한 원래 대상으로 진화가 이어집니다. 놓친 변이 기회는 `변이 재도전`(회당 1,000,000 토큰, 10% 확률)으로 다시 노려볼 수 있으며 같은 계보에서 30회 연속 실패하면 다음 도전에서 확정 발동합니다.
...
현재 게임에는 64종 시작형 가챠, Lv.15·25·40 레벨 진화와 갈림길 선택·변이·계승, 일반 진화에서 분리된 합성 수집품 10종, 6단계 진화 등급(`PROCESS` → `ORIGIN`), 유니크 컬러, 먹이 주기, 로컬 저장·복원과 256종 크리처 도감이 포함되어 있습니다.
...
## `13 // LICENSE BOUNDARY` — 라이선스와 아트워크

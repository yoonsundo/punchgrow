...
> **대상 버전:** v0.2.0 · Apple Silicon Mac · macOS 14 이상
...
- **`C n%`:** Claude의 실제 주간 플랜 한도 사용률입니다. v0.2.0은 `~/.claude/plugins/oh-my-claudecode/.usage-cache-anthropic.json` 캐시가 있을 때 이 값을 읽습니다.
...
- 잔액이나 먹이가 부족하거나 현재 크리처가 만렙이면 해당 동작이 중지됩니다.
...
- **성장 잠재력 ORIGIN:** 현재 자동 진화 경로를 끝까지 따라갔을 때 도달하는 최종 등급

현재 카탈로그에서 ORIGIN까지 성장하는 시작 계보는 60개 중 3개이므로 **ORIGIN 계보는 3/60, 즉 5%**입니다. 이것은 ORIGIN을 가챠에서 직접 뽑을 확률이 아닙니다. ORIGIN 계보를 뽑아도 처음에는 PROCESS이며 직접 성장시켜야 합니다.

![등급표](screenshots/rarity-guide.png)

하단 **등급표**에서는 직접 가챠 확률과 최종 성장 잠재력 비율을 따로 확인할 수 있습니다. `직접 0% · 진화 전용`은 해당 등급을 가챠에서 바로 얻을 수 없다는 뜻입니다.
...
먹이 한 번으로 여러 진화 기준을 한꺼번에 넘으면 가능한 단계까지 연속으로 진화합니다. 분기가 있는 계보에서는 게임 규칙이 선택한 자동 경로를 따릅니다.

![진화도감](screenshots/evolution-dex.png)

하단 **진화**를 누르면 현재 크리처의 시작형부터 최종형까지 볼 수 있습니다. `AUTO`는 실제 자동 진화 경로, `현재`는 지금 보유한 단계, 자물쇠는 아직 발견하지 않은 종을 뜻합니다.
...
| **등급표** | 직접 가챠 확률, 최종 잠재 등급별 계보 수, 보유·도감 수를 봅니다. |
| **진화** | 현재 크리처의 전체 계보와 자동 진화 경로를 봅니다. |
...
Connections에서 자동 수집이 실행 중인지 확인하고 Claude Code 또는 Codex를 한 번 사용한 뒤 다음 스캔을 기다리세요. `C —`는 v0.2.0이 읽는 `oh-my-claudecode` 사용량 캐시가 아직 생성되지 않았을 때도 계속 표시될 수 있습니다. `X —`는 Codex가 로컬 세션에 주간 한도 메타데이터를 아직 기록하지 않았을 수 있습니다. 정확한 값이 없으면 PunchGrow는 임의 비율을 만들지 않습니다.

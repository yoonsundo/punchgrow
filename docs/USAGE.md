...
3. 첫 실행에는 화면 오른쪽 아래에 데스크톱 펫이 함께 나타납니다. 아직 크리처가 없으면 첫 가챠 안내가 보입니다.
4. 메뉴 막대 표시를 클릭하면 작은 게임 화면이 열립니다.
5. 하단의 **설정**을 눌러 큰 창을 열고, 왼쪽 사이드바에서 **Connections**로 이동합니다.
6. **수집 동의 및 시작**을 누르면 약 10초 간격으로 로컬 사용량 변화를 확인합니다.
...
> [!IMPORTANT]
> Homebrew `v0.4.0`과 현재 `main`의 먹이 구성이 다릅니다. 배포본은 일반·대형 먹이를 제공하고, 아래의 대형·특대형 구성과 레거시 전환은 다음 릴리스 예정인 현재 `main`에만 있습니다.

| 동작 | 비용 또는 효과 |
| --- | --- |
| 가챠 1회 · Homebrew v0.4.0 | 500,000 토큰 · 64종 PROCESS 시작형 중 하나 |
| 가챠 1회 · 현재 `main` | 500,000 토큰 · 64종 PROCESS 시작형 중 하나 |
| Homebrew v0.4.0 · 일반 먹이 | 구매 100,000 토큰 · XP +25 · 친밀도 +3 |
| Homebrew v0.4.0 · 대형 먹이 | 구매 500,000 토큰 · XP +200 · 친밀도 +10 |
| 현재 `main` · 대형 먹이 | 구매 500,000 토큰 · XP +200 · 친밀도 +10 |
| 현재 `main` · 특대형 먹이 | 구매 2,500,000 토큰 · XP +1,000 · 친밀도 +50 |

- **가챠**는 한 번 클릭할 때 한 번 실행됩니다.
- Homebrew `v0.4.0`의 일반·대형 먹이 버튼은 360ms 동안 누르면 110ms 간격으로 반복을 시작하고, 실행할 때마다 8ms씩 줄어 최소 45ms까지 빨라집니다.
- 현재 `main`의 대형·특대형 **먹이 주기**와 **구매** 버튼은 한 번 클릭하거나 길게 누를 수 있습니다. 특대형 먹이는 가격과 효과가 모두 대형의 정확히 5배라 토큰당 효율이 같습니다.
- 길게 누르면 240ms 뒤 연속 실행을 시작합니다. 간격은 80ms에서 시작해 실행할 때마다 8ms씩 줄고 최소 35ms까지 빨라집니다. 손을 떼면 새 반복은 즉시 중단되며, 이미 저장 중인 1회가 있다면 그 저장만 마친 뒤 화면에 반영하고 더 실행하지 않습니다. 각 반복 결과는 저장 결과가 돌아온 뒤에만 화면에 나타납니다.
- 잔액이나 먹이가 부족하거나 만렙 크리처의 친밀도까지 100에 도달하면 반복이 중지됩니다. 만렙이어도 친밀도가 남아 있으면 먹이로 계속 올릴 수 있습니다. 진화 선택 대기 중인 크리처가 Lv.50에 닿은 경우에는 방향을 고르기 전까지 추가 급여가 막힙니다.
- 가챠로 새 크리처를 얻으면 그 개체가 현재 조작 대상이 됩니다. 대표 크리처는 **대표로 지정**을 눌렀을 때만 바뀝니다.

현재 `main`에서 이전 버전의 저장을 불러오면 보유한 일반 먹이는 5개당 대형 먹이 1개로 전환됩니다. 5개 미만으로 남은 일반 먹이는 1개당 100,000토큰으로 자동 환급되어 구매 가치가 사라지지 않습니다.
...
**데스크톱 펫 → 화면에 펫 표시**를 끄면 배경 없는 화면 위 크리처가 즉시 사라지고, 다시 켜면 마지막 위치에서 나타납니다. 대표 크리처가 있으면 대표와 고정한 표시 모습을 사용하며, 대표가 없을 때만 현재 크리처를 표시합니다. 펫을 드래그해 옮길 수 있고 위치·표시 여부는 이 Mac에만 저장됩니다. 앱 또는 시스템의 움직임 줄이기가 켜지면 펫의 대기 움직임도 멈춥니다.
...
- 파일 교체 뒤 디스크 동기화 확인에 실패하면 이미 반영된 결과는 화면에 유지하고 경고를 표시합니다. 이때 추가 게임 변경은 잠기며, **Data & Settings**에서 백업을 복원하면 다시 사용할 수 있습니다.

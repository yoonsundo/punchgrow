...
> **대상 버전:** v0.3.0 · Apple Silicon Mac · macOS 14 이상
...
> 이 문서의 화면은 앱이 직접 렌더링한 실제 SwiftUI 화면입니다. 재현 가능한 문서용 샘플 데이터를 사용했으며, 사용자의 로그나 개인 데이터는 포함하지 않습니다.
...
- **`C n%`:** Claude의 실제 주간 플랜 한도 사용률입니다. v0.3.0은 Claude Code의 로그인 정보를 읽기 전용으로 사용해 Anthropic 사용량 API에서 직접 조회하고, 조회할 수 없으면 `~/.claude/plugins/oh-my-claudecode/.usage-cache-anthropic.json` 캐시 값을 대신 읽습니다.
...
| 동작 | 비용 또는 효과 |
| --- | --- |
| 가챠 1회 | 500,000 토큰 · 60종 PROCESS 시작형 중 하나 |
...
현재 카탈로그에서 ORIGIN까지 도달 가능한 시작 계보는 60개 중 3개이므로 **ORIGIN 계보는 3/60, 즉 5%**입니다. 이것은 ORIGIN을 가챠에서 직접 뽑을 확률이 아니며, 실제로 ORIGIN까지 도달하려면 매 갈림길에서 그 방향을 직접 골라야 합니다. ORIGIN 계보를 뽑아도 처음에는 PROCESS이며 직접 성장시켜야 합니다.
...
일반 분기(60종 중 20종)는 진화 갈림길(Lv.15 또는 Lv.25, 개체당 평생 최대 1회, 2지선다)에서 방향을 직접 고릅니다. 진화도감에서 미리 **이 모습으로 키우기**를 눌러 예약해 두면 갈림길에서 멈추지 않고 예약한 모습으로 이어서 진화합니다. 예약하지 않은 채 갈림길에 닿으면 진화가 멈추고 **진화 선택 대기** 배지가 뜨며, 팝업에서 방향을 고를 때까지 그 상태로 남습니다.

변이 후보가 있는 계보는 Lv.15 진화 순간마다 10% 확률로 변이가 발동해 수락/거절을 묻습니다. 수락하면 그 자리에서 변이체(항상 2단계 종착)로 진화가 끝나고, 거절하면 원래 선택·예약해 둔 경로로 이어서 진화합니다. 발동을 놓쳤거나 다시 노려보고 싶다면 **변이 재도전**(회당 1,000,000 토큰, 10% 확률)을 쓸 수 있고, 같은 계보에서 30회 연속 실패하면 다음 도전에서 반드시 발동합니다.
...
하단 **진화**를 누르면 현재 크리처의 시작형부터 최종형까지 볼 수 있습니다. 진화도감 카드는 예약해 둔 대상을 강조해서 보여주며, `현재`는 지금 보유한 단계, 자물쇠는 아직 발견하지 않은 종을 뜻합니다.
...
| **진화** | 현재 크리처의 전체 계보와 선택 가능한 진화 경로, 예약 대상을 봅니다. |
